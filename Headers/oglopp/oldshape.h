...
namespace oglopp {
	...
	*/
	...
	protected:
		...
		*/
		OldShape& updateUniformMVP(Window& window, Shader* pShader);

		/** @brief Updated extra uniforms. obverloaded in each inherited class
		 * @return A reference to this shape object
	 	*/
		OldShape& updateExtraUniforms();
	...
	public:
		...
		* @return 		A reference to this shape object
		*/
		OldShape& pushPoint(glm::vec3 vec, glm::vec3 col, glm::vec2 texPos, float option);
		...
		* @return 		A reference to this shape object
		*/
		OldShape& pushPoint(glm::vec3 vec, glm::vec3 col, glm::vec2 texPos);
		...
		* @return 		A reference to this shape object
		*/
		OldShape& pushPoint(glm::vec3 vec, glm::vec3 col);
		...
		* @return 		A reference to this shape object
		*/
		OldShape& pushPoint(glm::vec3 vec, glm::vec2 texPos);

		/** @brief Push a single point to the shape.
		* @param[in]	vec		The vector of the point
		* @return 		A reference to this shape object
		*/
		OldShape& pushPoint(glm::vec3 vec);
		 ...
		 * @return 				A reference to this shape object
	 	*/
		OldShape& pushValue(void const* pValue, size_t bytes);
		 ...
		 * @return			A reference to this shape object
		*/
		OldShape& pushTriangle(unsigned int vertA, unsigned int vertB, unsigned int vertC);

		/** @brief Push a texture onto the back of the texture stack.
		 * @param[in] newTexture	The texture object to set to
		 * @return					A reference to this shape object
		*/
		OldShape& pushTexture(Texture* newTexture);
		 ...
		 * @return	A reference to this shape object
		*/
		OldShape& updateVAO(bool color = true, bool texture = true, bool option = false);
		 ...
		 * @return 	A reference to this shape object
	 	*/
		OldShape& finalizePoints(const int totalIndices);
		 ...
		 * @param[in] args...		Variadic list of arguments. Arguments are of type OldShape::DataType
		 * @return A reference to this shape object
	 	*/
		template <typename First, typename... Args>
		OldShape& finalizePoints(const int index, First firstParam, Args...args) {
			...
			uint32_t thisStrideElems = OldShape::getStrideElems(static_cast<DataType>(firstParam));
			uint32_t thisStrideBytes = thisStrideElems * OldShape::getStrideComponentBytes(static_cast<DataType>(firstParam));
			...
			switch(firstParam) {
				...
				case VEC3:
				case VEC4:
					glVertexAttribPointer(index, thisStrideElems, OldShape::getStructComponentRegister(firstParam), GL_FALSE, this->strideBytes, (void*)OFFSET);
					break;
				...
				case DVEC2:
				case DOUBLE:
					glVertexAttribLPointer(index, thisStrideElems, OldShape::getStructComponentRegister(firstParam), this->strideBytes, (void*)OFFSET);
					break;
				...
				case UVEC4:
				case U64VEC4:
					glVertexAttribIPointer(index, thisStrideElems, OldShape::getStructComponentRegister(firstParam), this->strideBytes, (void*)OFFSET);
					break;
		...
		/** @brief Replacement for updateVAO. Allows dynamically specifying the type of value. Termination case (DOES RECURSE)
		 * @param[in] firstParam	The first argument
		 * @param[in] ...			Variadic list of arguments. Arguments are of type OldShape::DataType
		 * @return A reference to this shape object
	 	*/
		template <typename First, typename... Args>
		OldShape& finalizePoints(First firstParam, Args...args) {
		...
		*/
		 ...
		 * @return 				A reference to this shape
		*/
		OldShape& draw(Window& window, Shader* pShader = nullptr);
